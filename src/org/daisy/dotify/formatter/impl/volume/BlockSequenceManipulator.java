...
class BlockSequenceManipulator {
    ...
    private BlockSequence newSequence(List<Block> c) {
        //TODO: this will break if the returned sequence attempts to use some methods
    ...
    private static Map<String, Integer> tagSequence(List<Block> seq) {
        ...
        for (Block group : seq) {
            if (
                group.getBlockIdentifier() != null &&
                !group.getBlockIdentifier().equals("") &&
                entries.put(group.getBlockIdentifier(), i) != null
            ) {
