...
class LayoutEngineImpl implements FormatterEngine {
    ...
    public LayoutEngineImpl(
        ...
        FactoryManager fm,
        ObflParserFactoryService obflFactoryService
    ) {
    ...
    public LayoutEngineImpl(
        ...
        PagedMediaWriter writer,
        FactoryManager fm,
        ObflParserFactoryService obflFactory
    ) {
        this.config = config;
        this.writer = writer;
    ...
    public void convert(InputStream input, OutputStream output) throws LayoutEngineException {
        ...
        try {
            try {
                ...
                Formatter formatter =
                        fm.getFormatterFactory().newFormatter(config.getLocale(), config.getTranslationMode());
                ...
                List<MetaDataItem> meta = obflParser.getMetaData().stream()
                        ...
                        // because the meta data in the OBFL file is about itself,
                        // and these properties are not transferable
                        .filter(item ->
                            !(
                                item.getKey().equals(DC_IDENTIFIER) ||
                                item.getKey().equals(DC_DATE) ||
                                item.getKey().equals(DC_FORMAT)
                            )
                        )
