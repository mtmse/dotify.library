...
class TableBorderHandler {
    ...
    String getSharedColumnString(Border cell1, Border cell2, BlockContext context) {
        ...
        if (tableColSpacing == 0) {
            ...
            sb.append(
                cell1 != null && cell1.getRight().getStyle() != Style.NONE ?
                    getStyle(posForRight(cell1.getRight().getAlign()), cell1.getRight().getWidth()) :
                    ""
            );
            sb.append(StringTools.fill(context.getFcontext().getSpaceCharacter(), tableColSpacing));
            sb.append(
                cell2 != null && cell2.getLeft().getStyle() != Style.NONE ?
                    getStyle(posForLeft(cell2.getLeft().getAlign()), cell2.getLeft().getWidth()) :
                    ""
            );
        }
        return sb.toString();
    ...
    private String mergedColumnStyles(Border c1, Border c2) {
        if (
            (c1 == null || c1.getRight().getStyle() == Style.NONE) &&
            (c2 == null || c2.getLeft().getStyle() == Style.NONE)
        ) {
            return "";
    ...
    String mergedRowStyles(Border c1, Border c2) {
        if (
            (c1 == null || c1.getBottom().getStyle() == Style.NONE) &&
            (c2 == null || c2.getTop().getStyle() == Style.NONE)
        ) {
            return "";
    ...
    private Position posForLeft(Align a) {
        switch (a) {
            ...
            case CENTER:
                return Position.CENTER_HORIZONTAL;
            default:
    ...
    Position posForTop(Align a) {
        switch (a) {
            ...
            case OUTER:
                return Position.TOP;
            case CENTER:
                return Position.CENTER_VERTICAL;
            default:
                throw new RuntimeException("Unexpected value. This is a bug.");
        }
    }
    ...
    String getStyle(Position align, int width) {
        ...
        if (style == null) {
            ...
            switch (align) {
                case LEFT:
                    return style.getLeftBorder();
                case RIGHT:
                    return style.getRightBorder();
                case TOP:
                    return style.getTopBorder();
                case BOTTOM:
                    return style.getBottomBorder();
