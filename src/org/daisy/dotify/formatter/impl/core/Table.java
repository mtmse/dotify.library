...
class Table extends Block {
    ...
    public void beginsTableBody() {
        //TODO: add check here that the table header has complete rows, i.e. no open rowspans
    ...
    protected AbstractBlockContentManager newBlockContentManager(BlockContext context) {
        ...
        Result r = minimizeCost(
            ...
            currentColumnWidth,
            colSpace,
            tableProps.getPreferredEmtpySpace(),
            context,
            dc,
            leftMargin,
            rightMargin
        );
        return new TableBlockContentManager(
            context.getFlowWidth(),
            r.minWidth,
            r.forceCount,
            r.rows,
            rdp,
    ...
    private Result minimizeCost(
        ...
        DefaultContext dc,
        MarginProperties leftMargin,
        MarginProperties rightMargin
    ) {
        ...
        //base result
        Result currentResult = renderTableWithCache(
            tableProps.getPreferredEmtpySpace(),
            currentColumnWidth,
            colSpacing,
            context,
        ...
        while (true) {
            ...
            for (int i = 0; i < columnCount; i++) {
                if (currentColumnWidth[i] >= 1) {
                    ...
                    try {
                        results[i] = renderTableWithCache(
                            spacePreferred,
                            currentColumnWidth,
                            colSpacing,
                            context,
            ...
            if (tableWidth <= maxTableWidth) {
                ...
                //since reduction is required at this point, choose the best new solution
                // (even if it is worse than the current)
                int start = x;
    ...
    private Result renderTableWithCache(
        int spacePreferred,
        int[] columnWidth,
        int[] colSpacing,
        BlockContext context,
        ...
        if (resultCache.containsKey(key)) {
            ...
            try {
                ...
                // Also put failing results in the resultCache to prevent them from being attempted again
                // (this is why finally is used)
    ...
    private Result updateRendering(
        int[] columnWidth,
        int[] colSpacing,
        TableCost costFunc,
        BlockContext context,
    ...
    private RowImpl getResultRow(
        ...
        for (int j = 0; j < td.getGridWidth(); j++) {
            ...
            if (cr.getInfo().getEndPoint().getCol() + 1 < td.getGridWidth()) {
                TableCell c = td.cellForGrid(
                    cr.getInfo().getStartingPoint().getRow(),
                    cr.getInfo().getEndPoint().getCol() + 1
                );
                String border;
                if (c == null) {
                    border = "";
                ...
                //TODO: length calculation incorrect if border is separated
                int length =
                    cr.getCellWidth() +
                    colSpacing[cr.getInfo().getEndPoint().getCol()] -
                    data.length() -
                    border.length();
                if (length > 0) {
    ...
    private class ColumnSpaceCalculator implements GridSpaceCalculator {
        ...
        public int getSpacingValue(TableCell cell1, TableCell cell2) {
            int b1 =
                    cell1.getInfo().getBorder() != null &&
                    getBorderAfter(cell1.getInfo().getBorder()).getStyle() != Style.NONE ?
                        1 : 0;
            int b2 =
                    cell2.getInfo().getBorder() != null &&
                    getBorderBefore(cell2.getInfo().getBorder()).getStyle() != Style.NONE ?
                        1 : 0;
