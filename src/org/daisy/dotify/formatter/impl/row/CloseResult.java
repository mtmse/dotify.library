...
class CloseResult implements CurrentResult {
    ...
    public Optional<RowImpl> process(SegmentProcessing spi, LineProperties lineProps) {
        if (cr.isPresent() && cr.get().hasNext(spi)) {
            return cr.get().process(spi, lineProps);
        } else if (doFlush) {
            doFlush = false;
            ...
            doUnderline = false;
            if (
                spi.getUnusedLeft() < spc.getMargins().getLeftMargin().getContent().length() ||
                spi.getUnusedRight() < spc.getMargins().getRightMargin().getContent().length()
            ) {
            ...
            return Optional.of(
                    new RowImpl.Builder(
                        StringTools.fill(
                                spc.getSpaceCharacter(),
                                spi.getUnusedLeft() - spc.getMargins().getLeftMargin().getContent().length()
                        ) +
                        StringTools.fill(
                            spc.getRdp().getUnderlineStyle(),
                            spc.getFlowWidth() - spi.getUnusedLeft() - spi.getUnusedRight()
                        )
                    )
