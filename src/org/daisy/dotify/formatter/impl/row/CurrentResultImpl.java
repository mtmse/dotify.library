...
class CurrentResultImpl implements CurrentResult {
        ...
        try {
            if (btr.hasNext()) { //LayoutTools.length(chars.toString())>0
                ...
                return startNewRow(
                    ...
                    btr,
                    "",
                    spc.getRdp().getTextIndent(),
                    spc.getRdp().getBlockIndent(),
                    mode,
                    lineProps
                );
            }
    ...
    private Optional<RowImpl> processFirst(SegmentProcessing spi, LineProperties lineProps) {
        ...
        if (!spi.hasCurrentRow()) {
            ...
            if (spi.hasListItem()) { //currentListType!=BlockProperties.ListType.NONE) {
                ...
                String listLabel;
                try {
                    listLabel = spc.getFormatterContext().getTranslator(mode).translate(
                        Translatable.text(
                ...
                try {
                    if (item.getType() == FormattingTypes.ListStyle.PL) {
                        return startNewRow(
                            ...
                            listLabel,
                            0,
                            spc.getRdp().getBlockIndentParent(),
                            mode,
                            lineProps
                        );
                    } else {
            ...
            return continueRow(
                spi,
                new RowInfo("", spc.getAvailable() - lineProps.getReservedWidth()),
                btr,
                spc.getRdp().getBlockIndent(),
                mode,
                lineProps
    ...
    private Optional<RowImpl> startNewRow(
        SegmentProcessing spi,
        BrailleTranslatorResult chars,
        ...
        return continueRow(
            spi,
            new RowInfo(
                getPreText(contentBefore, indent + blockIndent),
                spc.getAvailable() - lineProps.getReservedWidth()
            ),
            chars,
            blockIndent,
            mode,
    ...
    private Optional<RowImpl> continueRow(
        ...
        if (spi.getLeaderManager().hasLeader()) {
            ...
            int leaderPos = spi.getLeaderManager().getLeaderPosition(
                spc.getAvailable() - lineProps.getReservedWidth()
            );
            ...
            if (
                ...
                MarginProperties leftMargin = spi.getCurrentRow().getLeftMargin();
                if (spi.hasCurrentRow()) {
                    ret = spi.flushCurrentRow();
                }
                spi.newCurrentRow(leftMargin, spc.getMargins().getRightMargin());
                m1 = new RowInfo(getPreText(
                    "",
                    spc.getRdp().getTextIndent() + blockIndent),
                    spc.getAvailable() - lineProps.getReservedWidth()
                );
                //update offset
            ...
            try {
                tabSpace = spi.getLeaderManager().getLeaderPattern(
                    spc.getFormatterContext().getTranslator(mode),
                    offset - align
                );
            } finally {
    ...
    private void breakNextRow(
        ...
        String next = softHyphenPattern.matcher(
            btr.nextTranslatedRow(m1.getMaxLength(row) - contentLen, force, wholeWordsOnly)
        ).replaceAll("");
