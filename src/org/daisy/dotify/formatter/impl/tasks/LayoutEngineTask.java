...
public class LayoutEngineTask extends ReadWriteTask {
    ...
    private static PagedMediaWriter getWriter(
            Properties p2,
            TaskGroupSpecification spec,
            PagedMediaWriterFactoryMakerService pmw
    ) throws TaskSystemException {
    ...
    private static String getTranslationMode(
            Properties p2,
            FormatIdentifier out,
            String locale,
            BrailleTranslatorFactoryMakerService translatorFactory
    ) throws TaskSystemException {
            ...
            case Keys.PEF_FORMAT:
                return Optional.ofNullable(
                        Optional.ofNullable(p2.getProperty(TRANSLATE)).orElseGet(
                            () -> getDefaultMode(listSupportedModes(translatorFactory, locale)
    ...
    private static FormatterConfiguration getFormatterConfig(
            Properties p2,
        ...
        boolean allowsEndingVolumeOnHyphen = !p2.getProperty(ALLOWS_ENDING_VOLUME_ON_HYPHEN, "true")
    ...
    private static List<MetaDataItem> asMetadata(Properties p2) {
        ...
        for (Object key : p2.keySet()) {
            meta.add(new MetaDataItem.Builder(
                ENTRY,
                p2.get(key).toString()).attribute(new AttributeItem("key", key.toString())
            ).build());
        }
        return meta;
    }
    ...
    private static Stream<String> listSupportedModes(
            BrailleTranslatorFactoryMakerService translatorFactory,
            String locale
    ) {
        return translatorFactory.listSpecifications().stream()
                .filter(
                    v -> v.getLocale().equals(locale) &&
                        v.getModeDetails().getType().map(x -> !(x == TranslatorType.BYPASS ||
                        x == TranslatorType.PRE_TRANSLATED)).orElse(true).booleanValue()
                )
                .map(v -> v.getMode())
                .distinct();
    }
                ...
                try {
                    ...
                    for (ValidatorMessage m : vr.getMessages()) {
                        switch (m.getType()) {
                            ...
                            case FATAL_ERROR:
                            case WARNING:
                                // Using Level.WARNING for all validation messages
                                // (reserving Level.SEVERE for system errors)
                    ...
                    if (!vr.isValid()) {
                        logger.warning(
                            "The OBFL-file isn't valid! If this is systematic, " +
                            "please consider fixing your code. Invalid input will be rejected in future versions."
                        );
                        try {
                            Thread.sleep(10000);
