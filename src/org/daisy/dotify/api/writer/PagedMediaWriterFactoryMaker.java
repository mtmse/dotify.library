...
public class PagedMediaWriterFactoryMaker implements PagedMediaWriterFactoryMakerService {
    ...
    public static PagedMediaWriterFactoryMaker newInstance() {
        ...
        {
            Iterator<PagedMediaWriterFactoryService> i =
                ServiceLoader.load(PagedMediaWriterFactoryService.class).iterator();
            while (i.hasNext()) {
    ...
    public PagedMediaWriterFactory getFactory(String target) throws PagedMediaWriterConfigurationException {
                ...
                for (PagedMediaWriterFactoryService h : filters) {
                    if (h.supportsMediaType(target)) {
                        if (logger.isLoggable(Level.FINE)) {
                            logger.fine(
                                "Found a PagedMediaWriter factory for " + target + " (" + h.getClass() + ")"
                            );
                        }
        ...
        if (template == null) {
            throw new PagedMediaWriterFactoryConfigurationException(
                "Cannot find a PagedMediaWriter factory for " + target
            );
        }
