...
public class IndexV2Embosser extends IndexEmbosser {
    ...
    public EmbosserWriter newEmbosserWriter(OutputStream os) {
        ...
        if (numberOfCopies > maxNumberOfCopies || numberOfCopies < 1) {
            throw new IllegalArgumentException(
                new UnsupportedPaperException("Invalid number of copies: " + numberOfCopies + " is not in [1, 999]")
            );
        }
        //if (magazine && pageCount > 200) {
        //    throw new IllegalArgumentException(
        //      new UnsupportedPaperException("Number of pages = " + pageCount +
        //          "; cannot exceed 200 when in magazine style mode")
        //    );
        //}
    ...
    private byte[] getIndexV2Header(int cellsInWidth, boolean eightDots) {
        ...
        if (saddleStitchEnabled) {
            ...
            header.append('0');
        }              // 15: Page mode              = 1,2 or 4 pages per sheet or z-folding (3)
        header.append(",");
