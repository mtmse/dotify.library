...
public class IndexV3Embosser extends IndexEmbosser {
    ...
    private static final TableFilter TABLE_FILTER;
    ...
    public EmbosserWriter newEmbosserWriter(OutputStream os) {
        ...
        if (numberOfCopies > maxNumberOfCopies || numberOfCopies < 1) {
            throw new IllegalArgumentException(
                new EmbosserFactoryException(
                    "Invalid number of copies: " + numberOfCopies + " is not in [1, 10000]"
                )
            );
        }
    ...
    private byte[] getIndexV3Header() {
        ...
        switch (type) {
            ...
            case INDEX_BASIC_D_V3: {
                ...
                if (fPart > 0.75) {
                    xx = EmbosserTools.toBytes((int) (iPart + 1), 2);
                    y = '0';
                } else if (fPart > 2d / 3d) {
                    y = '5';
                } else if (fPart > 0.5) {
