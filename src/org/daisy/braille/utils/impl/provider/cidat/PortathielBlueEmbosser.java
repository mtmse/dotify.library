...
public class PortathielBlueEmbosser extends CidatEmbosser {
    ...
    private static final String transparentTable =
            "es_once_cidat.CidatTableProvider.TableType.PORTATHIEL_TRANSPARENT_6DOT";
    //private static final String transparent8dotTable =
    //        "es_once_cidat.CidatTableProvider.TableType.PORTATHIEL_TRANSPARENT_8DOT";
    ...
    static {
        tableFilter = new TableFilter() {
            ...
            public boolean accept(FactoryProperties object) {
                ...
                if (object.getIdentifier().equals(transparentTable)) {
                    return true;
                }
                //if (object.getIdentifier().equals(transparent8dotTable)) { return true; }
                if (object.getIdentifier().equals(mitTable)) {
                    return true;
                }
                if (object.getIdentifier().equals(nabccTable)) {
                    return true;
                }
                if (object.getIdentifier().equals(gbTable)) {
                    return true;
                }
                return false;
            }
    ...
    private byte[] getPortathielHeader(boolean duplex,
        ...
        if (pageLength < 8 || pageLength > 13) {
            throw new UnsupportedPaperException("Paper height = " + pageLength + " inches, must be in [8,13]");
        }
        if (charsPerLine < 12 || charsPerLine > 42) {
            throw new UnsupportedPaperException("Characters per line = " + charsPerLine + ", must be in [12,42]");
        }
        if (linesPerPage < 10 || linesPerPage > 31) {
        ...
        header.append("\r\u001b!DT");
        header.append(eightDots ? '8' : '6');                // 6 or 8 dots
        header.append("\r\u001b!DS");
        header.append(duplex ? '1' : '0');                   // Front-side or double-sided embossing
        header.append("\r\u001b!LM0");                                                  // Left margin
        header.append("\r\u001b!SL1");                                                  // Interline space = 1/10 inch
        header.append("\r\u001b!PL");
        bytes = EmbosserTools.toBytes(pageLength, 2);
        header.append((char) bytes[0]);
        header.append((char) bytes[1]);                   // Page length in inches
        header.append("\r\u001b!LP");
        bytes = EmbosserTools.toBytes(linesPerPage, 2);
        header.append((char) bytes[0]);
        header.append((char) bytes[1]);                   // Lines per page
        header.append("\r\u001b!CL");
        bytes = EmbosserTools.toBytes(charsPerLine, 2);
        header.append((char) bytes[0]);
