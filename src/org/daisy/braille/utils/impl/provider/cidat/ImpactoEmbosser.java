...
public class ImpactoEmbosser extends CidatEmbosser {
    ...
    private static final String transparentTable =
            CidatTableProvider.TableType.IDENTIFIER_PREFIX + "IMPACTO_TRANSPARENT_6DOT";
    //private static final String transparent8dotTable =
    //        CidatTableProvider.TableType.IDENTIFIER_PREFIX + "IMPACTO_TRANSPARENT_8DOT";
    ...
    private byte[] getImpactoHeader(boolean duplex,
        ...
        int linesPerPage = EmbosserTools.getHeight(printPage, getCellHeight());
            // depends on cell heigth -> depends on rowgap

        if (pageLength < 6 || pageLength > 13) {
            throw new UnsupportedPaperException("Paper height = " + pageLength + " inches, must be in [6,13]");
        }
        if (charsPerLine < 12 || charsPerLine > 42) {
            throw new UnsupportedPaperException("Characters per line = " + charsPerLine + ", must be in [12,42]");
        }
        if (linesPerPage < 12 || linesPerPage > 43) {
        ...
        header.append((char) 0x1b);
        header.append(eightDots ? '+' : '*');            // 6- or 8-dot matrix
        header.append((char) 0x1b);
        header.append('.');
        header.append((char) (0x30 + pageLength));    // Page length in inches
        header.append((char) 0x1b);
        header.append("/1");                         // Line spacing = 1/10 inch
        header.append((char) 0x1b);
        header.append('0');
        ...
        header.append((char) 0x1b);
        header.append('3');                          // Cut off words
        header.append((char) 0x1b);
        header.append(duplex ? 'Q' : 'P');               // Front-side or double-sided embossing
