...
public class EmbosserBrailleConverter implements BrailleConverter {
     ...
     */
    public EmbosserBrailleConverter(
        String table,
        Charset charset,
     ...
     * @param replacement    the replacement character, must be in the range 0x2800-0x283F
     * @param ignoreCase     set to true to ignore character case
     * @param t2bSupplements additional substitutions for text to braille translation. Values MUST be
     *                       braille characters.
     * @throws IllegalArgumentException if the table length isn't equal to 64 or 256 or if supplements values are not
     * braille characters.
     */
    public EmbosserBrailleConverter(
        String table,
            ...
            for (char key : t2bSupplements.keySet()) {
                ...
                if (val >= 0x2800 && val <= 0x28FF) {
                    ...
                    throw new IllegalArgumentException(
                        "Supplements value must be in range [0x2800, 0x28FF]: " + key + "=>" + val
                    );
                }
            }
        }
    ...
    private char toBraille(char c) {
        ...
        if (t2b.get(c) == null) {
            throw new IllegalArgumentException(
                    "Character '" + c + "' (0x" + Integer.toHexString((int) (c)) + ") not found."
            );
        }
        return (t2b.get(c));
    }
