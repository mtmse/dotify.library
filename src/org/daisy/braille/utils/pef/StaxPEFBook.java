...
class StaxPEFBook {
    ...
    private static final Logger logger = Logger.getLogger(StaxPEFBook.class.getCanonicalName());
    ...
    static PEFBook loadStax(URI uri, boolean compatibilityMode) {
        ...
        try {
            ...
            logger.throwing("StaxPEFBook", e.getMessage(), e);
        }
        return null;
    }
    ...
    private void scanMeta() throws XMLStreamException {
        if (
            !(
                ...
                META_TAG.equals(event.asStartElement().getName())
            )
        ) {
