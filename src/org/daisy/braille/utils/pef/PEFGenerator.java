...
public class PEFGenerator {
     ...
     * the number of volumes in the generated file.
     */
    public static final String KEY_VOLUMES = "volumes";
    /**
     * Key used in the settings map passed to the constructor. Its value defines
     * the number of sections in the generated file.
     */
    public static final String KEY_SPV = "sections";

    /**
     * Key used in the settings map passed to the constructor. Its value defines
     * the number of pages per volume in the generated file.
     */
    public static final String KEY_PPV = "pages";

    /**
     * Key used in the settings map passed to the constructor. Its value defines
     * if eight dot should be used (true/false).
     */
    public static final String KEY_EIGHT_DOT = "eightdot";
    /**
     * Key used in the settings map passed to the constructor. Its value defines
     * the maximum number of rows on a page in the generated file.
     */
    public static final String KEY_ROWS = "rows";
    /**
     * Key used in the settings map passed to the constructor. Its value defines
     * the maximum number of columns on a page in the generated file.
     */
    ...
    public void generateTestBook(File output) throws FileNotFoundException {
        ...
        pw.println(
            ...
            sdf.format((new Date())) + "</dc:identifier>"
        );
        ...
        pw.println(
            "\t\t\t<dc:description>Document generated for test purposes " +
            "containing random characters.</dc:description>"
        );
        pw.println("\t\t</meta>");
        pw.println("\t</head>");
        pw.println("\t<body>");
        ...
        for (int v = 0; v < volumes; v++) {
            ...
            pw.println(
                "\t\t<volume cols=\"" + cols + "\" rows=\"" +
                (rows + (int) Math.ceil((rows * rowgap) / 4d)) +
                "\" rowgap=\"" + rowgap + "\" duplex=\"" + duplex + "\">"
            );
            pw.println("\t\t\t<section>");
    ...
    public void generateTestPages(File output) throws FileNotFoundException,
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
        pw.println(
            "\t\t\t<dc:identifier>" + Integer.toHexString((int) (Math.random() * 1000000)) + " " +
            sdf.format((new Date())) + "</dc:identifier>"
        );
        pw.println("\t\t\t<dc:format>application/x-pef+xml</dc:format>");
        ...
        int rowgap = (eightDot ? 1 : 0);
        pw.println(
            "\t\t<volume cols=\"" + cols + "\" rows=\"" + (rows + (int) Math.ceil((rows * rowgap) / 4d)) +
            "\" rowgap=\"" + rowgap + "\" duplex=\"" + duplex + "\">"
        );
        pw.println("\t\t\t<section>");
    ...
    private List<String> centerAndBorder(List<String> content) throws Exception {
        ...
        for (String row : content) {
            ...
            sb.append(row);
            for (int j = 0; j < cols - row.length() - marginLeft - 2; j++) {
                sb.append(space);
            }
            sb.append(borderRight);
            page.add(sb.toString());
